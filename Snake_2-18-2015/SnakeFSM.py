...
def offensive_relations(degrees_differnence, distance, headx, heady):
	...
	degrees_around_goal = getAngle(snake.goal[0],snake.goal[1], headx, heady)
	...
	primex = int(snake.goal[0] + changex * distance)
	primey = int(snake.goal[1] + changey * distance)
...
class Attack(State):
	def strike(snakeA, snakegoal):
		snakeA = snakegoal

		return snakeA
	def retract(snakeA, snakegoal):
		pass
	def Execute(self):
		from Snake_Class import snake 
		from Snake_Class import timer 
		GoalAvector = Vector_Class.Vector(snake.goal, snake.A)
		head_cords = [snake.headx, snake.heady]
		headGoalvector = Vector_Class.Vector(head_cords, snake.goal)
		"""
		snake.headx, snake.heady = [snake.headx + headGoalvector.x, snake.heady + headGoalvector.y] 
		snake.Execute()
		timer.delta_time_Attack(5000)
		snake.headx, snake.heady = [snake.headx + headGoalvector.x / 2, snake.heady + headGoalvector.y / 2] 
		snake.Execute()
		timer.delta_time_Attack(5000)
		snake.headx, snake.heady = [snake.goal[0], snake.goal[1]] 
		timer.delta_time_Attack(20000)
		snake.A = [snake.A[0] + GoalAvector.x / 2, snake.A[1] + GoalAvector.y / 2]
		snake.Execute()
		timer.delta_time_Attack(5000)
		snake.A = [snake.A[0] + GoalAvector.x, snake.A[1] + GoalAvector.y]
		"""


		snake.A[0], snake.A[1] = [snake.goal[0], snake.goal[1] + 50] 
		snake.Execute()
		if timer.delta_time_Attack(3000) == True:
			print "Cake"
			snake.headx, snake.heady = [head_cords] 
			snake.Execute()
...
class SnakeFSM(object):
	...
	def __init__(self, char):	#
		self.char = char
		self.states = {"Traveling": Traveling(), "Offense": Offense(), "Attack":Attack()}
		self.transitions = {"To_Offense":Transition("Offense"), "To_Attack": Transition("Attack"), "To_Traveling": Transition("Traveling")}
		self.trans_name = {"To_Offense": "Offense", "To_Attack": "Attack", "To_Traveling":"Traveling"}
